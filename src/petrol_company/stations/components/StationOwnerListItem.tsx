...
// Phase 6 (US3): `ordersPerMonth` dropped — no per-client order-count aggregation exists
// anywhere on the platform (the same reasoning `OrdersListPage`'s header comment already
// states for order-level stat cards, FR-047).
...
interface StationOwnerListItemProps {
  owner: StationOwner;
  isLast?: boolean;
}

export function StationOwnerListItem({ owner, isLast }: StationOwnerListItemProps) {
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center justify-center gap-6 w-full max-w-xs bg-gray-100 rounded-2xl p-3 flex-1">
        ...
        <div className="flex flex-col items-center justify-center text-center min-w-[70px]">
          ...
          <span className="text-[10px] font-bold text-slate-500">{t('owners.stationsCount')}</span>
        </div>
      ...
      <div className="w-full md:w-auto flex justify-end shrink-0">
        <button
          onClick={() => navigate(`/petrolCompany/stations/owners/${owner.id}`)}
