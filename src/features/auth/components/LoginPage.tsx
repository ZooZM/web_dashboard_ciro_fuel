  ...
  const onSubmit = form.handleSubmit((values) => {
    if (!values.email && !values.password) {
      useSessionStore.getState().setSession(
        { id: '1', email: 'admin@ciro.com', name: 'إبراهيم القحطاني', role: 'admin' } as any,
        'dummy-token'
      );
    ...
    >
      ...
      <div className="flex-1 flex w-full max-w-7xl mx-auto items-center justify-start px-4 md:px-8 py-8 md:py-0">
        ...
        <div className="w-full max-w-[400px] bg-white/40 backdrop-blur-lg border border-white/60 rounded-[2.5rem] p-8 shadow-2xl flex flex-col gap-6 mt-8 md:mt-0">
          ...
          <form onSubmit={onSubmit} className="flex flex-col gap-4 w-full mt-2">
            ...
            <div className="space-y-1.5">
              <div className="relative flex items-center bg-white rounded-2xl border border-slate-200 focus-within:border-primary focus-within:ring-1 focus-within:ring-primary overflow-hidden shadow-sm h-14">
                ...
                <div className="flex items-center px-4 border-r border-slate-100 text-blue-600 bg-transparent h-full shrink-0 gap-1.5" dir="ltr">
                  <img src="/signIn/phone.svg" alt="Phone" className="w-4 h-4 ml-1 object-contain" />
                  <span className="text-xs font-medium mt-0.5">+966</span>
                  <ChevronDown className="w-3 h-3 mt-1.5 text-blue-600" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="relative flex items-center bg-white rounded-2xl border border-slate-200 focus-within:border-primary focus-within:ring-1 focus-within:ring-primary overflow-hidden shadow-sm h-14">
                <div className="flex-1 px-4 h-full relative flex items-center">
                  <Input 
                    id="password" 
                    type="password"
                    placeholder="كلمة المرور"
                    className="border-0 focus-visible:ring-0 shadow-none h-full rounded-none bg-transparent px-0 text-sm font-medium placeholder:text-slate-400"
                    {...form.register('password')}
                  />
                </div>
                <div className="flex items-center px-4 text-blue-600 h-full shrink-0 border-r border-slate-100">
                  <img src="/signIn/lock.svg" alt="Lock" className="w-4 h-4 object-contain" />
                </div>
              </div>
            </div>

            <div className="flex items-center flex-row-reverse justify-between mt-3 mb-2 px-1">
            ...
            >
              <img src="/signIn/logout.svg" alt="Login" className="w-5 h-5 object-contain" />
              تسجيل الدخول
            </Button>
      ...
      <div className="w-full max-w-7xl mx-auto px-4 md:px-8 pb-8 flex flex-col md:flex-row-reverse gap-6 justify-between mt-auto z-10">
        ...
        <div className="flex-1 bg-slate-100/90 backdrop-blur-xl border border-white/50 rounded-3xl p-5 flex items-center justify-between shadow-lg">
          <div className="text-right flex-1 pl-2">
            ...
            <p className="text-[10px] text-slate-500 leading-relaxed">ربط آمن بين المحطات و العلامات التجارية و الناقلون المعتمدون.</p>
          </div>
